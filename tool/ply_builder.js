var fs = require("fs"),
    ...
    output = (args[1]) ? args[1] : input;

fs.readdir(input, function (err, files) {
    if(err) throw err;

    files.forEach(function(fileName) {
        var fPath = input + "/" + fileName;

        if(fPath.indexOf(".ply") != -1) {
            var file = fs.readFileSync(fPath, "utf8");
            createJSONFile(fileName.split(".ply").join(""), file.toString());
        }
    });
});
...
function createJSONFile(name, str) {
    ...
    fs.readFile("ply_builder.tpl", "utf8", function(err, data) {
        if(err) {
        ...
        // 5. 파일 저장
        fs.writeFile(output + "/" + name + ".js", text, function(err) {
            if(err) {
                return console.log(err);
            }

            console.log("'" + output + "/" + name + ".js' created!!!");
        });
    });
}